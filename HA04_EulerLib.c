...
void eulerSettings_MSD(simHandle *handle)
{
    ...
    handle->stateVecInit = ((double *)malloc(sizeof(double) * (handle->numOfStates))); // missing check if storage is available: -1pkt
    ...
    handle->stateVec = (double *)malloc(sizeof(double) * ((int)ceil((handle->simTime / handle->stepSize) * (handle->numOfStates + 1)))); // missing check if storage is available: -1pkt
    ...
    for (int i = 0; i < (int)ceil((handle->simTime / handle->stepSize) * (handle->numOfStates)); i++)
    { //handle durchlaufen und alles auf 0 setzen
...
void showResults_MSD(simHandle *handle)
{
    ...
    /*call gnuplot*/
    FILE *gnuplotPipe = popen("gnuplot -persistent", "w"); // no plot there: -1pkt
}
